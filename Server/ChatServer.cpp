...
ChatServer::ChatServer(const quint16 port, QObject *parent)
    : QObject(parent) {
    ...
    if (m_server->listen(QHostAddress::Any, port)) {
        ...
        std::cerr << "[Error] Server failed: " << m_server->errorString().toStdString() << std::endl;
    }
}
...
void ChatServer::onNewConnection() {
    ...
    std::println(">>> NEW CONNECTION from: {}", clientAddress.toString().toStdString());
